...
2. 基本資訊
  ...
  2. 程式名稱：CLNS0\_B007
  3. URI：/common/batch/messageSend
  4. 類型：Spring Batch
  5. 運行平台：Paas 平台
...
- 1. boolean isHoliday = LIP-B-COMBUSSDQ001下行.WorkingDayFlag == 'N'; // true:假日 false:平日
  ...
  23. paramMap.put("INPUT\_DATE", @inputDate);
  24. extractstr = CommonUtil.replaceParam(map.EXTRACTSTR, extractstrParamMap);
...
- 1. }
  ...
  21. string msg = "";
  22. msg = CommonUtil.replaceParam(MSG, msgParamMap);
  ...
  27. extractstrMap.put("NEWS\_STATUS", statusSuccess);//預設成功
  28. //資料檢核
  29. Map chkMap = CommonUtil.chkMessageParams(extractstrMap);
  30. String isContinue = chkMap.isContinue;
  31. String errorMessage = chkMap.errorMessage;
  32. If(isContinue == 'Y'){
  33. Log.info(errorMessage);
  34. 結束本筆extractstrMap，繼續跑下一筆extractstrMap
  35. }
  36. If(notifyKind == 'A')){ // A: SMS 簡訊
  37. Map smsMap = new HashMap();
  38. smsMap.put("CaseNbr",APPLY\_ID);//案件編號
  39. smsMap.put("SMSNotifyContent",msg);//通知內容
  40. smsMap.put("SMSNotifySubject",SUBJECT);//通知主旨
  41. smsMap.put("NotifyCode",NOTIFY\_TYPE);//對外發信通知種類代碼
  42. smsMap.put("RecipientCellPhone", MOBILE);//收件人手機
  43. smsMap.put("RecipientId",INFM\_ID);//收件者統編
  44. smsMap.put("RecipientName",INFM\_NAME);//收件人姓名
  45. smsMap.put("AcntDeptCode", REMIT\_BRANCH);//費用分攤單位費用單位代號
  46. extractstrMapByKey.put(APPLY\_ID, NOTIFY\_TYPE, RecipientCellPhone,extractstrMap);//index
  47. smsList.add(smsMap); 新增進sms清單，後續呼叫通知API使用
  48. }else if(notifyKind in('B','C')){// B:INTERNAL-EMAIL 內部郵件// C: EXTERNAL-EMAIL 外部郵件 現實際無C情境
  49. List<Map> recipientList = new ArrayList<Map>();
  50. Map recipientMap = new HashMap();
  51. recipientMap.put("RecipientEmail",EMAIL);//收件人E-MAIL
  52. recipientMap.put("RecipientName",INFM\_NAME);//收件人姓名
  53. recipientMap.put("CcFlag",'N');// Y:副本/N:正本/B:密件副本
  54. recipientList.add(recipientMap);
  55. String mailNotifyCode = "";
  56. If(notifyKind == 'B'){ //1:行內2:行外
  57. mailNotifyCode = '1';
  58. }else{
  59. mailNotifyCode = '2';
  60. }
  61. Map emailMap = new HashMap();
  62. emailMap.put("CaseNbr",APPLY\_ID);//案件編號
  63. emailMap.put("MailNotifyCode", mailNotifyCode);// 1:行內2:行外
  64. emailMap.put("MailNotifyContent",msg);//信件通知內容
  65. emailMap.put("MailNotifySubject",SUBJECT);//信件通知主旨
  66. emailMap.put("MailTypeCode",'2');//發信類型 1:純文字 2:MIME(html標籤) 3:模版發信 4:BillHunter
  67. emailMap.put("NotifyCode",NOTIFY\_TYPE);// 對外發信通知種類代碼
  68. emailMap.put("RecipientList",recipientList);//收件人資訊
  69. extractstrMapByKey.put(APPLY\_ID, SUBJECT, EMAIL,extractstrMap);//index
  70. emailList.add(emailMap);//新增進email清單，後續呼叫通知API使用
  71. }else{
  72. Log.fatal("未支援類型 [" + TYPE + "]");
  73. 噴錯 結束批次
  74. }
  75. messageList.add(extractstrMap);新增進通知清單，計算兩百筆使用
  76. If(messageList不為空 && (messageList >= 200筆 or 本次迴圈已經是extractstrList最後一筆了)){//分批處理
  77. If(smsList不為空){
  78. 呼叫【LIP-C-CMSSDSMST001簡訊發送服務】API
  79. 傳入上行
...
- 1. //API結果情境:整批成功、整批失敗
  ...
  10. //新增log訊息通知紀錄
  11. CommonUtil.addMessageLog(messageList);
  12. cntMap.put("outputcnt", cntMap.get("outputcnt") + messageList.count);//紀錄輸出件數
  13. messageList清空;
  14. }// If(messageList不為空 && (messageList >= 200筆 or 本次迴圈已經是extractstrList最後一筆了)) 結尾
  15. } // for(extractstrMap : extractstrList){ 結尾
  16. }// if (Math.abs((inputMM - writeStrMM) % CYCLE\_MIN) <= 1) { 結尾
  17. }// if (inputTime >= writeStrTime && inputTime <= writeEndTime) { 結尾
  18. }// if ((!isHoliday || "Y".equals(WRITE\_HOLIDAY))…… 結尾
  19. } //for (map: messageSettingList) {結尾
  20. }// If(messageSettingList 不為空){ 結尾
1. 紀錄執行結束時間
